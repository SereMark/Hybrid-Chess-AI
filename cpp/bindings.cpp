...
namespace {
  ...
  inline void
  check_2d_float32(const py::buffer_info& info, const char* name, ssize_t expected_cols) {
    if (info.ndim != 2)
...
PYBIND11_MODULE(chesscore, m) {
  ...
  py::class_<chess::Move>(m, "Move")
      ...
      .def(py::init<int, int, chess::Piece>(),
           py::arg("from_sq"),
           py::arg("to_sq"),
           py::arg("promotion"))
  ...
  py::class_<mcts::MCTS>(m, "MCTS")
      ...
      .def("set_dirichlet_params",
           &mcts::MCTS::set_dirichlet_params,
           py::arg("alpha"),
           py::arg("weight"))
      .def("set_c_puct_params",
           &mcts::MCTS::set_c_puct_params,
           py::arg("c_puct_base"),
           py::arg("c_puct_init"))
  ...
  m.def(
      ...
      [](const std::vector<chess::Position>& positions) {
        ...
        py::array_t<float> a(
            {B, static_cast<ssize_t>(planes), static_cast<ssize_t>(H), static_cast<ssize_t>(W)});
        auto         info   = a.request();
        auto*        ptr    = static_cast<float*>(info.ptr);
