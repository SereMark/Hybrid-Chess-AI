...
inline void check_2d_float32(const py::buffer_info &info, const char *name,
                             ssize_t expected_cols) {
  if (info.ndim != 2)
    throw py::value_error(std::string(name) + " must be 2D [B,N]");
  if (expected_cols >= 0 && info.shape[1] != expected_cols) {
    std::ostringstream oss;
    oss << name << " has wrong size: expected [B," << expected_cols
        << "], got [B," << info.shape[1] << "]";
...
PYBIND11_MODULE(chesscore, m) {
  ...
  py::class_<chess::Move>(m, "Move")
      ...
      .def("__repr__",
           [](const chess::Move &mv) {
             return std::string("Move(") + sq_to_str(mv.from()) + "->" +
                    sq_to_str(mv.to()) +
      ...
      .def("__int__",
           [](const chess::Move &mv) { return static_cast<int>(mv.data); })
      .def("__hash__",
           [](const chess::Move &mv) { return py::hash(py::int_(mv.data)); })
      .def("__eq__",
           [](const chess::Move &a, const chess::Move &b) { return a == b; });
  ...
  py::class_<chess::Position>(m, "Position")
      ...
      .def("__repr__",
           [](const chess::Position &p) {
             return std::string("Position(") + p.to_fen() + ")";
           })
            ...
            auto eval_fn = [&evaluator](
              ...
              if (!py::isinstance<py::tuple>(out))
                throw py::value_error(
                    "evaluator() must return (policy, value)");
  ...
  m.def(
      "uci_of_move", [](const chess::Move &mv) { return move_to_uci(mv); },
      py::arg("move"));
}
