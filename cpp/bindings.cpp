...
inline std::string sq_to_str(int sq) {
  if (sq < 0 || sq >= chess::NSQUARES) return "--";
...
inline void check_2d_float32(const py::buffer_info& info, const char* name, ssize_t expected_cols) {
  if (info.ndim != 2) throw py::value_error(std::string(name) + " must be 2D [B,N]");
  if (expected_cols >= 0 && info.shape[1] != expected_cols) {
    std::ostringstream oss;
    oss << name << " has wrong size: expected [B," << expected_cols << "], got [B," << info.shape[1] << "]";
    throw py::value_error(oss.str());
  }
  if (info.itemsize != static_cast<ssize_t>(sizeof(float))) throw py::value_error(std::string(name) + " must be float32");
}
inline void check_value_shape(const py::buffer_info& info, ssize_t batch) {
  if (info.ndim == 1) {
    if (info.shape[0] != batch) throw py::value_error("value length mismatch");
  } else if (info.ndim == 2) {
    if (info.shape[0] != batch || info.shape[1] != 1) throw py::value_error("value must be [B,1] if 2D");
  } else {
    throw py::value_error("value must be [B] or [B,1]");
  }
  if (info.itemsize != static_cast<ssize_t>(sizeof(float))) throw py::value_error("value must be float32");
}
}  // namespace
...
PYBIND11_MODULE(chesscore, m) {
  ...
  py::class_<chess::Move>(m, "Move")
      ...
      .def("__repr__", [](const chess::Move& mv){ return std::string("Move(") + sq_to_str(mv.from()) + "->" + sq_to_str(mv.to()) + (mv.promotion()!=chess::PIECE_NONE? ", promo":"") + ")"; })
  ...
  py::class_<chess::Position>(m, "Position")
      ...
      .def("legal_moves", [](chess::Position &pos){
        chess::MoveList moves = pos.legal_moves();
        std::vector<chess::Move> out;
        out.reserve(moves.size());
        for (size_t i = 0; i < moves.size(); ++i) out.push_back(moves[i]);
        return out;
      })
      ...
      .def(py::pickle(
        [](const chess::Position& p){ return p.to_fen(); },
        [](const std::string& fen){ chess::Position p; p.from_fen(fen); return p; }
      ));
  ...
  m.def("encode_position",
        [](const chess::Position &pos) {
          ...
          { py::gil_scoped_release rel; encoder::encode_position_into(pos, ptr); }
          return a;
        },
        py::arg("position"));
  ...
  m.def("encode_batch",
        [](const std::vector<std::vector<chess::Position>> &histories) {
          ...
          { py::gil_scoped_release rel;
            for (ssize_t i = 0; i < B; ++i)
              encoder::encode_position_with_history(histories[static_cast<size_t>(i)], ptr + stride * static_cast<size_t>(i));
          }
          return a;
        },
        py::arg("histories"));
