...
using Square = int;
using Hash = uint64_t;
constexpr int BOARD_SIZE = 8, NSQUARES = 64, MAX_MOVES_PER_POSITION = 256, NUM_PIECE_TYPES = 6, NUM_COLORS = 2,
              NCASTLING = 16;
...
constexpr uint8_t ALL_CASTLING_RIGHTS = 15, WHITE_KINGSIDE = 1, WHITE_QUEENSIDE = 2, BLACK_KINGSIDE = 4,
                  BLACK_QUEENSIDE = 8;
...
constexpr Bitboard WHITE_KINGSIDE_CLEAR = 0x60ULL, WHITE_QUEENSIDE_CLEAR = 0xEULL,
                   BLACK_KINGSIDE_CLEAR = 0x6000000000000000ULL, BLACK_QUEENSIDE_CLEAR = 0xE00000000000000ULL;
...
struct MoveList {
    ...
    [[gnu::always_inline]] void add(Square f, Square t, Piece p = PIECE_NONE) noexcept {
        if (count < MAX_MOVES_PER_POSITION)
            moves[count++] = Move(f, t, p);
    }
...
class alignas(64) Position {
  public:
    ...
    [[nodiscard]] inline Color get_turn() const noexcept {
        return turn;
    }
    ...
    [[nodiscard]] inline Hash get_hash() const noexcept {
        return hash;
    }
