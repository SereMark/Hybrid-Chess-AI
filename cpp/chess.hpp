...
using Square = int;
using Hash = uint64_t;
constexpr int BOARD_SIZE = 8, NSQUARES = 64, MAX_MOVES_PER_POSITION = 256,
              NUM_PIECE_TYPES = 6, NUM_COLORS = 2, NCASTLING = 16;
...
constexpr uint8_t ALL_CASTLING_RIGHTS = 15, WHITE_KINGSIDE = 1,
                  WHITE_QUEENSIDE = 2, BLACK_KINGSIDE = 4, BLACK_QUEENSIDE = 8;
...
struct MoveList {
  ...
  [[gnu::always_inline]] void add(Square f, Square t,
                                  Piece p = PIECE_NONE) noexcept {
    if (count < MAX_MOVES_PER_POSITION)
