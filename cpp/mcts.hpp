...
namespace mcts {
  ...
  struct alignas(32) Node {
    ...
    [[gnu::hot, gnu::always_inline]] float
    ucb(float c_puct, float sqrt_visits, float parent_q, float fpu) const;
  ...
  public:
    ...
    std::vector<int>
    search_batched(const chess::Position& position, EvalBatchFn eval_fn, int max_batch = 64);
  ...
  private:
    ...
    [[gnu::hot]] void
    expand_node(Node* node, const chess::MoveList& moves, const std::vector<float>& policy);
    [[gnu::hot]] void                   add_dirichlet_noise(Node* node);
    [[gnu::always_inline]] static float get_policy_value(const chess::Move&        m,
                                                         const std::vector<float>& p) {
      const int idx = encode_move_index(m);
      return (idx >= 0 && idx < static_cast<int>(p.size())) ? p[static_cast<size_t>(idx)]
                                                            : POLICY_EPSILON;
    }
  };
}
