...
struct alignas(32) Node {
    // Children are stored in a contiguous pool; this is the index of the first child
    ...
    [[gnu::hot, gnu::always_inline]] float ucb(float c_puct, float sqrt_visits, float parent_q,
                                               float fpu) const;
...
class MCTS {
  public:
    ...
    // Evaluator signature: given positions, fill softmax policy logits and scalar value per item
