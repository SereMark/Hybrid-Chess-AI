...
class MCTS {
  public:
    ...
    using EvalBatchFn = std::function<void(const std::vector<chess::Position> &, std::vector<std::vector<float>> &,
                                           std::vector<float> &)>;
  ...
  private:
    ...
    [[gnu::hot]] void expand_node(Node *node, const chess::MoveList &moves, const std::vector<float> &policy);
    [[gnu::hot]] void add_dirichlet_noise(Node *node);
    [[gnu::always_inline]] static float get_policy_value(const chess::Move &m, const std::vector<float> &p) {
        const int idx = encode_move_index(m);
        return (idx >= 0 && idx < static_cast<int>(p.size())) ? p[static_cast<size_t>(idx)] : POLICY_EPSILON;
    }
};
} // namespace mcts
