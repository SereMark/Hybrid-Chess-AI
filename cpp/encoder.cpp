...
void encode_position_into(const chess::Position &pos, float *out) {
  ...
  for (int t = 0; t < history_length; t++) {
    ...
    for (int piece = 0; piece < 6; piece++) {
      for (int color = 0; color < 2; color++) {
        ...
        chess::Bitboard bb =
            pos.get_pieces(static_cast<chess::Piece>(piece), static_cast<chess::Color>(color));
        while (bb) {
  ...
  const float fullmove_val = std::min(1.0f, static_cast<float>(pos.get_fullmove()) / 100.0f);
  std::fill(out + static_cast<size_t>(fullmove_plane) * plane_area,
            out + static_cast<size_t>(fullmove_plane + 1) * plane_area, fullmove_val);
...
void encode_position_with_history(const std::vector<chess::Position> &history, float *out) {
  ...
  const int available = std::min(history_length, static_cast<int>(history.size()));
  ...
  const float fullmove_val = std::min(1.0f, static_cast<float>(cur.get_fullmove()) / 100.0f);
  std::fill(out + static_cast<size_t>(fullmove_plane) * plane_area,
            out + static_cast<size_t>(fullmove_plane + 1) * plane_area, fullmove_val);
  ...
  const int halfmove_plane = castling_start_plane + 4;
  const float halfmove_val = std::min(1.0f, static_cast<float>(cur.get_halfmove()) / 100.0f);
  std::fill(out + static_cast<size_t>(halfmove_plane) * plane_area,
            out + static_cast<size_t>(halfmove_plane + 1) * plane_area, halfmove_val);
