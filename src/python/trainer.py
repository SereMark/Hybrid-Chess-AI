"""Training orchestration components for Hybrid Chess AI."""
...
class ArenaResult:
    """Summarizes the outcome of an arena gating session."""
...
class Trainer:
    """Coordinates neural network training, self-play, and evaluation."""
    ...
    def __init__(self, device: str | None = None, resume: bool = False) -> None:
        ...
        total_expected_train_steps = int(
            C.TRAIN.TOTAL_ITERATIONS * C.TRAIN.LR_SCHED_STEPS_PER_ITER_EST
        )
        warmup_steps_clamped = int(
            max(1, min(C.TRAIN.LR_WARMUP_STEPS, max(1, total_expected_train_steps - 1)))
        )
        restart_interval_iters = int(
            max(0, getattr(C.TRAIN, "LR_RESTART_INTERVAL_ITERS", 0))
        )
        if restart_interval_iters > 0:
            steps_per_iter_est = float(
                max(1.0, float(C.TRAIN.LR_SCHED_STEPS_PER_ITER_EST))
            )
            restart_interval_steps = int(
                max(1, round(restart_interval_iters * steps_per_iter_est))
            )
        else:
        ...
        try:
            self.evaluator.set_batching_params(
                self._eval_batch_cap, self._eval_coalesce_ms
            )
        except Exception as exc:
            self.log.debug(
                "Failed to apply evaluator batching params: %s", exc, exc_info=True
            )
        try:
            self.evaluator.set_cache_capacity(int(C.EVAL.CACHE_CAPACITY))
        except Exception as exc:
            self.log.debug(
                "Failed to apply evaluator cache capacity: %s", exc, exc_info=True
            )
        ...
        ratio_min = float(
            getattr(C.SAMPLING, "TRAIN_RECENT_SAMPLE_RATIO_MIN", ratio_max)
        )
        self._recent_sample_ratio: float = float(
            np.clip(ratio_max, ratio_min, ratio_max)
        )
        ...
        try:
            ...
            self.log.debug(
                "Self-play adjudication priming failed: %s", exc, exc_info=True
            )
        try:
            self.selfplay_engine.set_game_length(C.ARENA.GAME_MAX_PLIES)
        except Exception as exc:
            self.log.debug(
                "Unable to set self-play game length: %s", exc, exc_info=True
            )
        self.iteration = 0
        self.total_games = 0
            ...
            if C.LOG.METRICS_LOG_CSV_ENABLE:
                ...
                if log_dir:
                    try:
                        ...
                        self.log.debug(
                            "Unable to create metrics directory %s: %s",
                            log_dir,
                            exc,
                            exc_info=True,
                        )
    ...
    def _gate_state_metrics(
        self, expected_games: int, games_played: int
    ) -> dict[str, Any]:
    ...
    def _maybe_run_arena(
        self, iteration: int, expected_games: int, run_eval: bool
    ) -> ArenaResult:
        ...
        natural_like = (
            term_natural + term_resign + term_threefold + term_fifty + term_loop
        )
        ...
        natural_ratio = (
            (natural_like / max(1, total_reason_games)) if total_reason_games else 0.0
        )
        if (
        ...
        metrics.update(
            {
                ...
                "term_natural_pct": (
                    100.0 * natural_like / total_reason_games
                    if total_reason_games
                    else 0.0
                ),
                "term_adjudicated_pct": (
                    100.0 * term_adjudicated / total_reason_games
                    if total_reason_games
                    else 0.0
                ),
                "term_loop_pct": (
                    100.0 * term_loop / total_reason_games
                    if total_reason_games
                    else 0.0
                ),
    ...
    def _restore_best_weights(self) -> None:
        ...
        if self.ema is not None:
            self.ema.shadow = {
                k: v.detach().clone() for k, v in base_best.state_dict().items()
            }
        self.optimizer = build_optimizer(self.model)
        self.scheduler.opt = self.optimizer
        current_lr = (
            self.scheduler._lr_at(self.scheduler.t)
            if hasattr(self.scheduler, "_lr_at")
        ...
        self._sync_selfplay_evaluator()
        self.log.info(
            "[Arena   ] challenger rejected; restored best weights for continued training"
        )
    ...
    def _append_csv_row(
        ...
        if not path:
            return
        reporter = (
            self.metrics if path == self.metrics.csv_path else MetricsReporter(path)
        )
    ...
    def train_step(
        self,
        batch_data: tuple[
            list[Any], list[np.ndarray], list[np.ndarray], list[np.ndarray]
        ],
    ...
    def train(self) -> None:
        ...
        for iteration in range(self.iteration + 1, C.TRAIN.TOTAL_ITERATIONS + 1):
            ...
            if self.device.type == "cuda":
                ...
                peak_alloc = 0.0
                peak_res = 0.0
            mem_info_summary = get_mem_info(
                self._proc, self.device, self.device_total_gb
            )
            try:
                ...
                can_decrease = alloc_frac > 0.90 and prev_bs > int(
                    C.TRAIN.BATCH_SIZE_MIN
                )
                if can_increase:
                    self.train_batch_size = int(
                        min(int(C.TRAIN.BATCH_SIZE_MAX), prev_bs + 128)
                    )
                elif can_decrease:
                    self.train_batch_size = int(
                        max(int(C.TRAIN.BATCH_SIZE_MIN), prev_bs - 256)
                    )
            ...
            sp_loop_reset_buffer = int(iter_stats.get("sp_loop_reset_buffer", 0))
            sp_loop_reset_reason = str(iter_stats.get("sp_loop_reset_reason", ""))
            ...
            resign_threshold = _as_float(
                iter_stats.get("resign_threshold"), C.RESIGN.VALUE_THRESHOLD
            )
            resign_min_plies = int(
                iter_stats.get(
                    "resign_min_plies", getattr(C.RESIGN, "MIN_PLIES_FINAL", 0)
                )
            )
            ...
            reset_segment = (
                f"reset={sp_loop_auto_reset}/{sp_loop_reset_count} "
                f"cd={sp_loop_reset_cd} pct={sp_loop_reset_pct:.0f}%"
            )
            if sp_loop_reset_buffer:
                reset_segment += f" buf={sp_loop_reset_buffer}"
            if sp_loop_reset_reason:
                reset_segment += f" reason={sp_loop_reset_reason}"
            sp_loop_line = (
            ...
            if arena_result.ran:
                ...
                arena_decisive_pct_val = (
                    100.0 * (arena_w + arena_l) / arena_n if arena_n else 0.0
                )
                arena_line = (
            ...
            if C.LOG.METRICS_LOG_CSV_ENABLE:
                try:
                    ...
                    fieldnames = [
                        ...
                        "sp_loop_reset_pct",
                        "sp_loop_reset_buffer",
                        "sp_loop_reset_reason",
                    ...
                    arena_decisive_pct = (
                        ((arena_w + arena_l) / arena_n * 100.0) if arena_n > 0 else 0.0
                    )
                    ...
                    row = {
                        ...
                        "batches_per_sec": float(
                            iter_stats.get("batches_per_sec", 0.0)
                        ),
                        "samples_per_sec": float(samples_per_sec),
                        "train_time_s": train_time,
                        "train_steps_planned": int(
                            iter_stats.get("train_steps_planned", 0)
                        ),
                        "train_steps_actual": int(
                            iter_stats.get("train_steps_actual", 0)
                        ),
                        "train_samples": int(
                            iter_stats.get("train_samples", opt_steps * train_bs)
                        ),
                        "samples_ratio": float(iter_stats.get("samples_ratio", 0.0)),
                        ...
                        "lr_sched_drift_pct": float(
                            iter_stats.get("lr_sched_drift_pct", 0.0)
                        ),
                        "train_recent_pct": float(
                            iter_stats.get("train_recent_pct", 0.0)
                        ),
                        ...
                        "sp_white_win_pct": float(
                            iter_stats.get("sp_white_win_pct", 0.0)
                        ),
                        ...
                        "sp_draw_true_pct": float(
                            iter_stats.get("sp_draw_true_pct", 0.0)
                        ),
                        "sp_draw_cap_pct": float(
                            iter_stats.get("sp_draw_cap_pct", 0.0)
                        ),
                        "sp_black_wins": int(iter_stats.get("black_wins", 0)),
                        "sp_black_win_pct": float(
                            iter_stats.get("sp_black_win_pct", 0.0)
                        ),
                        "sp_decisive_pct": (
                            100.0
                            * (
                                int(iter_stats.get("white_wins", 0))
                                + int(iter_stats.get("black_wins", 0))
                            )
                            / max(1, int(iter_stats.get("games", 0)))
                        ),
                        "sp_gpm": float(iter_stats.get("games_per_min", 0.0)),
                        "sp_mps_k": float(iter_stats.get("moves_per_sec", 0.0))
                        / 1000.0,
                        "sp_avg_len": sp_len,
                        "sp_new_moves": int(
                            iter_stats.get("sp_new_moves", iter_stats.get("moves", 0))
                        ),
                        "sp_white_starts": int(iter_stats.get("sp_white_starts", 0)),
                        "sp_black_starts": int(iter_stats.get("sp_black_starts", 0)),
                        "sp_white_start_pct": float(
                            iter_stats.get("sp_white_start_pct", 0.0)
                        ),
                        "sp_black_start_pct": float(
                            iter_stats.get("sp_black_start_pct", 0.0)
                        ),
                        "sp_color_bias_black_pct": float(
                            iter_stats.get("sp_color_bias_black_pct", 0.0)
                        ),
                        "sp_color_bias_target_pct": float(
                            iter_stats.get("sp_color_bias_target_pct", 0.0)
                        ),
                        "sp_terminal_eval_mean": float(
                            iter_stats.get("sp_terminal_eval_mean", 0.0)
                        ),
                        "sp_terminal_eval_abs_mean": float(
                            iter_stats.get("sp_terminal_eval_abs_mean", 0.0)
                        ),
                        "sp_tempo_bonus_avg": float(
                            iter_stats.get("sp_tempo_bonus_avg", 0.0)
                        ),
                        "sp_sims_avg": float(iter_stats.get("sp_sims_avg", 0.0)),
                        "sp_unique_positions_avg": float(
                            iter_stats.get("sp_unique_positions_avg", 0.0)
                        ),
                        "sp_unique_ratio_pct": float(
                            iter_stats.get("sp_unique_ratio_pct", 0.0)
                        ),
                        "sp_halfmove_avg": float(
                            iter_stats.get("sp_halfmove_avg", 0.0)
                        ),
                        "sp_halfmove_max": int(iter_stats.get("sp_halfmove_max", 0)),
                        "sp_threefold_pct": float(
                            iter_stats.get("sp_threefold_pct", 0.0)
                        ),
                        "sp_threefold_games": int(
                            iter_stats.get("sp_threefold_games", 0)
                        ),
                        "sp_threefold_draws": int(
                            iter_stats.get("sp_threefold_draws", 0)
                        ),
                        "sp_loop_games": int(iter_stats.get("sp_loop_games", 0)),
                        "sp_loop_alerts": int(iter_stats.get("sp_loop_alerts", 0)),
                        "sp_loop_pct": float(iter_stats.get("sp_loop_pct", 0.0)),
                        "sp_loop_flag_games": int(
                            iter_stats.get("sp_loop_flag_games", 0)
                        ),
                        "sp_loop_flag_pct": float(
                            iter_stats.get("sp_loop_flag_pct", 0.0)
                        ),
                        "sp_loop_unique_ratio_pct": float(
                            iter_stats.get("sp_loop_unique_ratio_pct", 0.0)
                        ),
                        ...
                        "sp_loss_eval_mean": float(
                            iter_stats.get("sp_loss_eval_mean", 0.0)
                        ),
                        "sp_loss_eval_p05": float(
                            iter_stats.get("sp_loss_eval_p05", 0.0)
                        ),
                        "sp_loss_eval_p25": float(
                            iter_stats.get("sp_loss_eval_p25", 0.0)
                        ),
                        ...
                        "sp_adjudicate_margin": float(
                            iter_stats.get("sp_adjudicate_margin", 0.0)
                        ),
                        "sp_adjudicate_min_plies": int(
                            iter_stats.get("sp_adjudicate_min_plies", 0)
                        ),
                        "sp_term_natural": int(iter_stats.get("sp_term_natural", 0)),
                        "sp_term_exhausted": int(
                            iter_stats.get("sp_term_exhausted", 0)
                        ),
                        "sp_term_resign": int(iter_stats.get("sp_term_resign", 0)),
                        "sp_term_threefold": int(
                            iter_stats.get("sp_term_threefold", 0)
                        ),
                        "sp_term_fifty": int(iter_stats.get("sp_term_fifty", 0)),
                        "sp_term_adjudicated": int(
                            iter_stats.get("sp_term_adjudicated", 0)
                        ),
                        "sp_term_loop": int(iter_stats.get("sp_term_loop", 0)),
                        "sp_term_natural_pct": float(
                            iter_stats.get("sp_term_natural_pct", 0.0)
                        ),
                        "sp_term_exhausted_pct": float(
                            iter_stats.get("sp_term_exhausted_pct", 0.0)
                        ),
                        "sp_term_resign_pct": float(
                            iter_stats.get("sp_term_resign_pct", 0.0)
                        ),
                        "sp_term_threefold_pct": float(
                            iter_stats.get("sp_term_threefold_pct", 0.0)
                        ),
                        "sp_term_fifty_pct": float(
                            iter_stats.get("sp_term_fifty_pct", 0.0)
                        ),
                        "sp_term_adjudicated_pct": float(
                            iter_stats.get("sp_term_adjudicated_pct", 0.0)
                        ),
                        "sp_term_loop_pct": float(
                            iter_stats.get("sp_term_loop_pct", 0.0)
                        ),
                        "sp_adjudicated_games": int(
                            iter_stats.get("sp_adjudicated_games", 0)
                        ),
                        "sp_adjudicated_white": int(
                            iter_stats.get("sp_adjudicated_white", 0)
                        ),
                        "sp_adjudicated_black": int(
                            iter_stats.get("sp_adjudicated_black", 0)
                        ),
                        "sp_adjudicated_draw": int(
                            iter_stats.get("sp_adjudicated_draw", 0)
                        ),
                        "sp_adjudicated_pct": float(
                            iter_stats.get("sp_adjudicated_pct", 0.0)
                        ),
                        "sp_adjudicated_white_pct": float(
                            iter_stats.get("sp_adjudicated_white_pct", 0.0)
                        ),
                        "sp_adjudicated_black_pct": float(
                            iter_stats.get("sp_adjudicated_black_pct", 0.0)
                        ),
                        "sp_adjudicated_draw_pct": float(
                            iter_stats.get("sp_adjudicated_draw_pct", 0.0)
                        ),
                        ...
                        "buffer_size": int(
                            iter_stats.get(
                                "buffer_size",
                                getattr(self.selfplay_engine, "size", lambda: 0)(),
                            )
                        ),
                        ...
                        "eval_requests_total": int(
                            iter_stats.get(
                                "eval_requests_total",
                                eval_metrics_now.get("requests_total", 0),
                            )
                        ),
                        "eval_cache_hits_total": int(
                            iter_stats.get(
                                "eval_cache_hits_total",
                                eval_metrics_now.get("cache_hits_total", 0),
                            )
                        ),
                        "eval_requests_delta": int(
                            iter_stats.get("eval_requests_delta", 0)
                        ),
                        "eval_cache_hits_delta": int(
                            iter_stats.get("eval_cache_hits_delta", 0)
                        ),
                        "eval_hit_rate": float(iter_stats.get("eval_hit_rate", 0.0)),
                        "eval_hit_rate_delta": float(
                            iter_stats.get("eval_hit_rate_delta", 0.0)
                        ),
                        "eval_batches_total": int(
                            iter_stats.get(
                                "eval_batches_total",
                                eval_metrics_now.get("batches_total", 0),
                            )
                        ),
                        "eval_batches_delta": int(
                            iter_stats.get("eval_batches_delta", 0)
                        ),
                        "eval_positions_total": int(
                            iter_stats.get(
                                "eval_positions_total",
                                eval_metrics_now.get("eval_positions_total", 0),
                            )
                        ),
                        "eval_positions_delta": int(
                            iter_stats.get("eval_positions_delta", 0)
                        ),
                        "eval_batch_size_max": int(
                            iter_stats.get(
                                "eval_batch_size_max",
                                eval_metrics_now.get("batch_size_max", 0),
                            )
                        ),
                        "eval_batch_cap": int(
                            iter_stats.get("eval_batch_cap", self._eval_batch_cap)
                        ),
                        "eval_coalesce_ms": int(
                            iter_stats.get("eval_coalesce_ms", self._eval_coalesce_ms)
                        ),
                        "eval_avg_batch": float(iter_stats.get("eval_avg_batch", 0.0)),
                        ...
                        "arena_decisive_games": int(
                            _as_float(arena_metrics.get("decisive"), 0.0)
                        ),
                        "arena_margin_target": _as_float(
                            arena_metrics.get("margin_target"), C.ARENA.GATE_BASELINE_P
                        ),
                        "arena_term_resign": _as_float(
                            arena_metrics.get("term_resign"), 0.0
                        ),
                        "arena_term_natural": _as_float(
                            arena_metrics.get("term_natural"), 0.0
                        ),
                        "arena_term_exhausted": _as_float(
                            arena_metrics.get("term_exhausted"), 0.0
                        ),
                        "arena_term_threefold": _as_float(
                            arena_metrics.get("term_threefold"), 0.0
                        ),
                        "arena_term_fifty": _as_float(
                            arena_metrics.get("term_fifty"), 0.0
                        ),
                        "arena_term_adjudicated": _as_float(
                            arena_metrics.get("term_adjudicated"), 0.0
                        ),
                        "arena_term_loop": _as_float(
                            arena_metrics.get("term_loop"), 0.0
                        ),
                        "arena_term_natural_pct": _as_float(
                            arena_metrics.get("term_natural_pct"), 0.0
                        ),
                        "arena_term_adjudicated_pct": _as_float(
                            arena_metrics.get("term_adjudicated_pct"), 0.0
                        ),
                        "arena_term_loop_pct": _as_float(
                            arena_metrics.get("term_loop_pct"), 0.0
                        ),
                        ...
                        "arena_expected_games": _as_float(
                            arena_metrics.get("expected_games"), float(expected_games)
                        ),
                        "arena_actual_games": _as_float(
                            arena_metrics.get("actual_games"), float(games_played)
                        ),
                        "arena_gate_z": _as_float(
                            arena_metrics.get("gate_z"), self._gate.z
                        ),
                        "arena_gate_w": _as_float(
                            arena_metrics.get("gate_w"), self._gate.w
                        ),
                        "arena_gate_d": _as_float(
                            arena_metrics.get("gate_d"), self._gate.d
                        ),
                        "arena_gate_l": _as_float(
                            arena_metrics.get("gate_l"), self._gate.losses
                        ),
                        "arena_gate_games": _as_float(
                            arena_metrics.get("gate_games"),
                            self._gate.w + self._gate.d + self._gate.losses,
                        ),
                        ...
                        "arena_gate_min_decisive": _as_float(
                            arena_metrics.get("gate_min_decisive"),
                            self._gate.min_decisive,
                        ),
                        "arena_gate_force_decisive": _as_float(
                            arena_metrics.get("gate_force_decisive"),
                            1.0 if self._gate.force_decisive else 0.0,
                        ),
                        ...
                        "arena_gate_baseline_margin": _as_float(
                            arena_metrics.get("gate_baseline_margin"),
                            self._gate.baseline_margin,
                        ),
                        "arena_gate_started_iter": _as_float(
                            arena_metrics.get("gate_started_iter"),
                            self._gate_started_iter,
                        ),
                        ...
                        "gpu_allocated_gb": float(
                            mem_info_summary.get("allocated_gb", 0.0)
                        ),
                        "gpu_reserved_gb": float(
                            mem_info_summary.get("reserved_gb", 0.0)
                        ),
