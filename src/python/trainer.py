...
class Trainer:
    ...
    def _run_arena(self) -> ArenaResult | None:
        ...
        promotion_threshold = float(getattr(C.ARENA, "promotion_score_pct", 50.0))
        if float(result.score_pct) >= promotion_threshold:
            self.best_model.load_state_dict(candidate_model.state_dict(), strict=True)
            save_best_model(self)
            decision = f"accepted (score {result.score_pct:.1f}% >= {promotion_threshold:.1f}%)"
        else:
            decision = f"rejected (score {result.score_pct:.1f}% < {promotion_threshold:.1f}%)"
