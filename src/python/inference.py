...
class BatchedEvaluator:
    ...
    def infer_positions_legal(
        ...
        if counts is not None:
            if not isinstance(moves_per_position, np.ndarray):
                raise TypeError("If the counts argument is provided, moves_per_position must be a numpy array")
    ...
    def _dispatch_eval(self, positions_list: list[Any], moves_list: list[Any]) -> tuple[list[np.ndarray], np.ndarray]:
        ...
        if not request.event.wait(timeout):
            # Batching should help throughput, not leave one unlucky caller
            # waiting forever. If our request is still queued, handle it here.
            removed = False
            with self._cv:
