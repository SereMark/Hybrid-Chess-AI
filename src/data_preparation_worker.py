...
from src.utils.chess_utils import convert_board_to_input,get_move_mapping

class DataPreparationWorker:
    def __init__(self,raw_pgn,max_games,min_elo,max_elo,batch_size,engine_path,engine_depth,engine_threads,engine_hash,pgn_file,max_opening_moves,skip_min_moves=0,skip_max_moves=99999,use_time_analysis=False,analysis_time=0.5):
    ...
    def run(self):
        ...
        with chess.engine.SimpleEngine.popen_uci(self.engine_path) as engine:
            ...
            with h5py.File(h5_path,"w") as h5_file:
                h5_file.create_dataset("inputs",(0,25,8,8),maxshape=(None,25,8,8),dtype=np.float32,compression="lzf")
                ...
                h5_file.create_dataset("fen_history",(0,),maxshape=(None,),dtype=dt,compression="lzf")
                skipped_games=0
                ...
                with open(self.raw_pgn_file,"r",errors="ignore") as f:
                    while self.total_games_processed<self.max_games:
                        ...
                        if tc:self.time_control_stats[tc]+=1
                        ...
                        while node.variations:
                            ...
                            if move_count<self.skip_min_moves or move_count>self.skip_max_moves:
                                ...
                                node=next_node
                                continue
                            inp=convert_board_to_input(board)
                            mid=self.move_mapping.get_index_by_move(move)
                            if mid is None:
                                board.push(move)
                                node=next_node
                                continue
                            limit=chess.engine.Limit(time=self.analysis_time) if self.use_time_analysis else chess.engine.Limit(depth=self.engine_depth)
                            try:
                            ...
                            self.batch_fens.append(board.fen())
                            if self.augment_flip:
                                fb=board.mirror()
                                fm=chess.Move(chess.square_mirror(move.from_square),chess.square_mirror(move.to_square),promotion=move.promotion)
                                fm_id=self.move_mapping.get_index_by_move(fm)
                                if fm_id is not None:
                                    fi=convert_board_to_input(fb)
                                    self.batch_inputs.append(fi)
                                    self.batch_policy_targets.append(fm_id)
                                    self.batch_value_targets.append(-fv)
                                    self.batch_fens.append(fb.fen())
                            if self.augment_mirror_rank:
                                p=board.fen().split()
                                rb=chess.Board("/".join(reversed(p[0].split("/")))+" "+" ".join(p[1:]))
                                rm=chess.Move(chess.square(chess.square_file(move.to_square),7-chess.square_rank(move.to_square)),chess.square(chess.square_file(move.from_square),7-chess.square_rank(move.from_square)),promotion=move.promotion)
                                rm_id=self.move_mapping.get_index_by_move(rm)
                                if rm_id is not None:
                                    ri=convert_board_to_input(rb)
                                    self.batch_inputs.append(ri)
                                    self.batch_policy_targets.append(rm_id)
                                    self.batch_value_targets.append(-fv)
                                    self.batch_fens.append(rb.fen())
                            board.push(move)
                            node=next_node
    ...
    def _write_batch_to_h5(self,h5_file,batch_table):
        ...
        try:
            ...
            h5_file["fen_history"].resize((end_index,))
            fen_array=np.array(self.batch_fens,dtype=object)
            h5_file["fen_history"][self.current_dataset_size:end_index]=fen_array
    ...
    def _log_game_stats_to_wandb(self,game_table,skipped,progress):
        try:
            ...
            wandb.log({"games_processed":self.total_games_processed,"games_skipped":skipped,"progress":progress,"current_batch_size":len(self.batch_inputs),"total_dataset_size":self.current_dataset_size})
        except:pass
    def _generate_opening_book(self):
        if not self.pgn_file or not self.max_opening_moves:return
        skipped_opening=0
        last_update=time.time()
            ...
            while self.game_counter<self.max_games:
                ...
                try:
                    ...
                    for count,move in enumerate(game.mainline_moves(),1):
                        if count>self.max_opening_moves:break
                        fen=board.fen()
                        uci_move=move.uci()
                    ...
                    if self.game_counter%10==0 or time.time()-last_update>5:
                        ...
                        try:
                            wandb.log({"opening_games_processed":self.game_counter,"opening_games_skipped":skipped_opening,"opening_progress":progress,"unique_positions":len(self.positions)})
                        except:pass
                        last_update=time.time()
                except:
                    skipped_opening+=1
    ...
    def _create_train_val_test_split(self,h5_path):
        with h5py.File(h5_path,"r") as hf:
            ...
            wandb.log({"train_size":len(splits["train"]),"val_size":len(splits["val"]),"test_size":len(splits["test"])})
