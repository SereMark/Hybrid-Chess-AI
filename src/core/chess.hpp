...
constexpr Square WHITE_KING_START = E1, WHITE_KING_KINGSIDE = G1, WHITE_KING_QUEENSIDE = C1,
                 WHITE_ROOK_KINGSIDE_START = H1, WHITE_ROOK_KINGSIDE_END = F1,
                 WHITE_ROOK_QUEENSIDE_START = A1, WHITE_ROOK_QUEENSIDE_END = D1;

constexpr Square BLACK_KING_START = E8, BLACK_KING_KINGSIDE = G8, BLACK_KING_QUEENSIDE = C8,
                 BLACK_ROOK_KINGSIDE_START = H8, BLACK_ROOK_KINGSIDE_END = F8,
                 BLACK_ROOK_QUEENSIDE_START = A8, BLACK_ROOK_QUEENSIDE_END = D8;

constexpr Bitboard WHITE_KINGSIDE_CLEAR = 0x60ULL, WHITE_QUEENSIDE_CLEAR = 0xEULL,
                   BLACK_KINGSIDE_CLEAR = 0x6000000000000000ULL, BLACK_QUEENSIDE_CLEAR = 0x0E00000000000000ULL;
...
struct Move {
  ...
  [[nodiscard]] bool operator==(const Move& o) const noexcept { return data == o.data; }
};
...
struct MoveList {
  ...
  inline void add(Move m) noexcept {
    if (count < MAX_MOVES_PER_POSITION) moves[count++] = m;
  }
  inline void add(Square from, Square to, Piece promo = PIECE_NONE) noexcept {
    if (count < MAX_MOVES_PER_POSITION) moves[count++] = Move(from, to, promo);
  }
  inline void clear() noexcept { count = 0; }
  [[nodiscard]] inline size_t size() const noexcept { return count; }
  ...
  inline Move* begin() noexcept { return moves; }
  inline Move* end() noexcept { return moves + count; }
  inline const Move* begin() const noexcept { return moves; }
  inline const Move* end() const noexcept { return moves + count; }
};
...
public:
  ...
  Result                    result();
  [[nodiscard]] inline int  repetition_count() const { return count_repetitions(); }
  ...
  [[nodiscard]] inline uint8_t get_castling() const noexcept { return castling; }
  [[nodiscard]] inline Square  get_ep_square() const noexcept { return ep_square; }
  [[nodiscard]] inline uint16_t get_halfmove() const noexcept { return halfmove; }
...
private:
  ...
  void               parse_game_state_from_fen(const std::string&, const std::string&, const std::string&, uint16_t, uint16_t);
...
[[nodiscard]] inline Square lsb(Bitboard b) noexcept { return static_cast<Square>(std::countr_zero(b)); }
inline int popcount(Bitboard b) noexcept { return static_cast<int>(std::popcount(b)); }
inline Bitboard bit(Square s) noexcept { return (s >= 0 && s < NSQUARES) ? (1ULL << s) : 0ULL; }
inline void pop_lsb(Bitboard& b) noexcept { b &= b - 1; }
