...
class SupervisedWorker:
    def __init__(self, epochs:int, batch_size:int, learning_rate:float, weight_decay:float, checkpoint_interval:int, dataset_path:str, train_indices_path:str, val_indices_path:str, model_path:Optional[str]=None, optimizer_type:str='adamw', scheduler_type:str='cosineannealingwarmrestarts', num_workers:int=4, random_seed:int=42, progress_callback=None, status_callback=None):
    ...
    def run(self):
        ...
        for epoch in range(start_epoch, self.epochs + 1):
            train_epoch(model=self.model, data_loader=train_loader, device=self.device, scaler=self.scaler, optimizer=self.optimizer, scheduler=self.scheduler, epoch=epoch, accumulation_steps=max(256 // self.batch_size, 1), batch_size=self.batch_size, smooth_policy_targets=True, compute_accuracy_flag=True, progress_callback=self.progress_callback, status_callback=self.status_callback)
