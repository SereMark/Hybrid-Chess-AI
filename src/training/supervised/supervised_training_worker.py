...
class SupervisedWorker:
    def __init__(self, epochs:int, batch_size:int, learning_rate:float, weight_decay:float, checkpoint_interval:int, dataset_path:str, train_indices_path:str, val_indices_path:str, model_path:Optional[str]=None, optimizer_type:str='adamw', scheduler_type:str='cosineannealingwarmrestarts', accumulation_steps:int=3, num_workers:int=4, random_seed:int=42, policy_weight:float=1.0, value_weight:float=2.0, grad_clip:Optional[float]=0.1, progress_callback=None, status_callback=None):
        ...
        self.model_path, self.optimizer_type, self.scheduler_type, self.grad_clip = model_path, optimizer_type, scheduler_type, grad_clip
    ...
    def run(self):
        ...
        for epoch in range(start_epoch, self.epochs + 1):
            train_epoch(model=self.model, data_loader=train_loader, device=self.device, scaler=self.scaler, optimizer=self.optimizer, scheduler=self.scheduler, epoch=epoch, accumulation_steps=self.accumulation_steps, batch_size=self.batch_size, smooth_policy_targets=True, compute_accuracy_flag=True, progress_callback=self.progress_callback, status_callback=self.status_callback, policy_weight=self.policy_weight, value_weight=self.value_weight, max_grad_norm=self.grad_clip)
