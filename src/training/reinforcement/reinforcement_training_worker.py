    ...
    def run(self):
        ...
        for iteration in range(self.start_iteration,self.num_iterations+1):
            if self.status_callback:
                self.status_callback("🔁 Iteration {}/{} 🎮 Generating self-play data...".format(iteration,self.num_iterations))
            ...
            for i in range(num_processes):
                ...
                tasks.append((model_state,self.device.type,self.simulations_per_move,self.c_puct,self.temperature,local_games,seeds[i]))
            ...
            if all_inputs:
                ...
                data_loader=DataLoader(TensorDataset(inputs_tensor,policy_tensor,value_tensor),batch_size=self.batch_size,shuffle=True,pin_memory=(self.device.type=="cuda"),num_workers=self.num_workers,persistent_workers=(self.num_workers>0))
                ...
                for ep in range(1,self.epochs_per_iteration+1):
                    train_metrics=train_epoch(self.model,data_loader,self.device,self.scaler,self.optimizer,self.scheduler,ep,self.epochs_per_iteration,self.accumulation_steps,False,self.policy_weight,self.value_weight,self.grad_clip,self.progress_callback,self.status_callback,self.wandb_flag)
            ...
            for idx,game in enumerate(pgn_games,start=1):
                filename="data/games/self-play/game_{}_{}.pgn".format(int(time.time()),idx)
