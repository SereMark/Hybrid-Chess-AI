...
class ReinforcementTrainingSubTab(BaseTab):
    ...
    def start_self_play(self):
        ...
        model_exists = model_path and os.path.isfile(model_path)
        ...
        started = self.start_worker(
            ...
            filters=filters if not model_exists else 64,
            res_blocks=res_blocks if not model_exists else 5,
            inplace_relu=inplace_relu if not model_exists else True
        )
