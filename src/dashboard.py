...
def validate_path(path: str, path_type: str = "file") -> bool:
    ...
    if path_type == "file": return os.path.isfile(path)
    elif path_type == "directory": return os.path.isdir(path)
    return False

def input_with_validation(label: str, default_value: str = "", path_type: str = "file", help_text: str = None, key: str = None,) -> str:
    unique_key = key or (label + "_key").replace(" ", "_").lower()
    path = st.text_input(label, default_value, help=help_text, key=unique_key)

    if path:
        if validate_path(path, path_type): st.markdown(f"✅ **Valid Path**")
        else: st.markdown(f"⚠️ **Invalid Path**")
    return path
...
def data_preparation_tab():
    ...
    raw_pgn = input_with_validation(
        ...
        default_value="data/raw/lichess_db_standard_rated_2025-03.pgn",
        path_type="file",
...
def lichess_deployment_tab():
    ...
    st.write("Configure and deploy your bot to play on Lichess.")
    ...
    if use_mcts:
        mcts_simulations = st.slider("MCTS Simulations:", min_value=100, max_value=2000, value=800, step=100, key="lichess_mcts_simulations")
        mcts_c_puct = st.slider("MCTS c_puct:", min_value=0.1, max_value=5.0, value=1.4, step=0.1, key="lichess_mcts_c_puct")
    else:
        mcts_simulations = None
        mcts_c_puct = None
    auto_resign = st.checkbox("Enable Auto-Resign", value=False, key="lichess_auto_resign")
...
if __name__ == "__main__":
    ...
    with tabs[0]: data_preparation_tab()
    with tabs[1]: supervised_training_tab()
    with tabs[2]: reinforcement_training_tab()
    with tabs[3]: evaluation_tab()
    with tabs[4]: benchmarking_tab()
    with tabs[5]: hyperparameter_optimization_tab()
    with tabs[6]: lichess_deployment_tab()