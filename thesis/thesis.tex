...
\begin{enumerate}
  ...
  \item \textbf{Teljes tanítási ciklus demonstrálása} egynapos időkerettel.
...
Minden kísérleti futás \textsc{YAML}-alapú konfigurációs fájlokból indul, amelyeket a \path{main.py} belépési pont \texttt{--config} és \texttt{--override} kapcsolókkal rétegez egymásra.
A konfigurációkezelő komponens minden futás elején egy \emph{végleges}, már felülbírált konfigurációt állít elő, amelyet a rendszer a \path{runs/<azonosító>/config/merged.{yaml,json}} állományokba ment.
...
A Python-környezet főbb csomagjait a \path{requirements.txt}-ben rögzítettem.
...
Amennyiben a \texttt{SEED} értéke nem nulla, a \path{main.py} a tanítás indításakor inicializálja a Python beépített \texttt{random} modulját, a NumPy véletlenszám-generátorát, valamint a \texttt{torch} CPU- és GPU-RNG-it a megadott maggal.
Ezzel párhuzamosan beállítja a \texttt{torch.use\_deterministic\_algorithms(True)} opciót, a \texttt{CUBLAS\_WORKSPACE\_CONFIG} környezeti változót, letiltja a cuDNN adaptív \texttt{benchmark} módját, illetve a logikai processzorszámhoz igazítja az intra- és inter-op szálak számát.
...
A \path{tools/benchmarks.py} modulban az inferencia-benchmark rögzített \texttt{2025}-ös Torch-maggal fut, az önjátszás áteresztőképességét mérő komponens a \texttt{SelfPlayEngine} belső RNG-jét \texttt{123}-as maggal inicializálja, míg a C++ sakkmag- és MCTS-benchmarkok véletlen FEN-állásait szintén paraméterezhető, alapértelmezésben \texttt{2025}-ös mag vezérli.
Az arénamérkőzésekre épülő bajnokság NumPy \texttt{default\_rng(2025)} generátorral készül, így a párosítások sorrendje és a lépésválasztások sztochasztikus komponense is kontrollált.
...
A \path{tools/generate_artifacts.py} szkript ezeket az állományokat olvassa be, a konfigurációs metaadatokkal együtt, majd egységes betűtípus- és stílusbeállítások mellett automatikusan készíti el az ábrákat (tanulási dinamika, skálázódási görbék, játszmahossz-eloszlás, bajnoki Elo-eredmények).
...
\begin{enumerate}
  \item A kód és a \path{requirements.txt} alapján a futtatási környezet felépítése, azonos vagy hasonló GPU-val.
  \item A kívánt konfiguráció kiválasztása (pl.\ \path{configs/run4_high_exploration.yaml}) és a teljes tanítás lefuttatása a \path{main.py} szkripttel, opcionálisan rögzített \texttt{SEED} értékkel.
  \item A benchmark-szkriptek (\path{tools/benchmarks.py}) futtatása opcionális teljesítménymérésekhez.
  \item Az ábrák teljes automatikus újragenerálása a \path{tools/generate_artifacts.py} eszközzel, amely a \path{runs/} könyvtárból olvassa ki a metrikákat.
\end{enumerate}
...
A cél a módszertan megvalósíthatóságának demonstrálása volt, nem pedig versenyképes motor fejlesztése.
