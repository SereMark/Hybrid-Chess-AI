...
class Config:
    blocks: int = _env_int("BLOCKS", 10)
    channels: int = _env_int("CHANNELS", 256)
    input_planes: int = 119
    policy_output: int = 73 * 64
    planes_per_position: int = 14

    simulations_train: int = _env_int("SIMULATIONS_TRAIN", 256)
    ...
    learning_rate_schedule: tuple = ((400, 0.01), (800, 0.001), (1200, 0.0001))
    ...
    games_per_iteration: int = _env_int("GAMES_PER_ITERATION", 600)
    buffer_size: int = _env_int("BUFFER_SIZE", 300_000)
    ...
    train_steps_per_iter: int = _env_int("TRAIN_STEPS_PER_ITER", 1000)
    ...
    selfplay_workers: int = _env_int("SELFPLAY_WORKERS", 8)
    eval_max_batch: int = _env_int("EVAL_MAX_BATCH", 256)
    eval_batch_timeout_ms: int = _env_int("EVAL_BATCH_TIMEOUT_MS", 2)
