...
PYBIND11_MODULE(chessai, m) {
    ...
    py::class_<chess::Move>(m, "Move")
        ...
        .def(py::init<int, int>(), "Create move from-to",
             py::arg("from_sq"), py::arg("to_sq"))
        ...
        .def_property_readonly(
            ...
            [](const chess::Position &pos) {
                ...
                for (int p = 0; p < 6; p++) {
                    py::list inner;
                    inner.append(
                        pos.get_pieces(static_cast<chess::Piece>(p),
                                       chess::WHITE));
                    inner.append(
                        pos.get_pieces(static_cast<chess::Piece>(p),
                                       chess::BLACK));
    ...
    py::class_<mcts::MCTS>(m, "MCTS")
        ...
        .def("set_c_puct", &mcts::MCTS::set_c_puct,
             "Set exploration constant", py::arg("c"))
