...
static inline void set_plane_value(float* out, int plane, int row, int col) {
  const size_t board_area = static_cast<size_t>(chess::BOARD_SIZE) * static_cast<size_t>(chess::BOARD_SIZE);
  out[static_cast<size_t>(plane) * board_area + static_cast<size_t>(row) * static_cast<size_t>(chess::BOARD_SIZE) +
...
static inline void fill_repetition_planes(const std::vector<chess::Position>& hist, float* out) {
  ...
  for (int t = 0; t < lim; ++t) {
    ...
    for (int j = start; j <= idx; ++j) {
      if (hist[static_cast<size_t>(j)].get_hash() == h) ++reps;
    }
    if (reps >= 2) std::fill(out + static_cast<size_t>(base + 12) * A, out + static_cast<size_t>(base + 13) * A, 1.0f);
    if (reps >= 3) std::fill(out + static_cast<size_t>(base + 13) * A, out + static_cast<size_t>(base + 14) * A, 1.0f);
  }
}
void encode_position_into(const chess::Position& pos, float* out) {
  ...
  std::fill(out, out + total_elems, 0.0f);
  {
    ...
    if (reps >= 2)
      std::fill(out + static_cast<size_t>(base + 12) * board_area, out + static_cast<size_t>(base + 13) * board_area,
                1.0f);
    if (reps >= 3)
      std::fill(out + static_cast<size_t>(base + 13) * board_area, out + static_cast<size_t>(base + 14) * board_area,
                1.0f);
  }
  ...
  if (c & chess::WHITE_KINGSIDE)
    std::fill(out + static_cast<size_t>(c0 + 0) * board_area, out + static_cast<size_t>(c0 + 1) * board_area, 1.0f);
  if (c & chess::WHITE_QUEENSIDE)
    std::fill(out + static_cast<size_t>(c0 + 1) * board_area, out + static_cast<size_t>(c0 + 2) * board_area, 1.0f);
  if (c & chess::BLACK_KINGSIDE)
    std::fill(out + static_cast<size_t>(c0 + 2) * board_area, out + static_cast<size_t>(c0 + 3) * board_area, 1.0f);
  if (c & chess::BLACK_QUEENSIDE)
    std::fill(out + static_cast<size_t>(c0 + 3) * board_area, out + static_cast<size_t>(c0 + 4) * board_area, 1.0f);
