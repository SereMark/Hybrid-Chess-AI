...
class MCTS {
public:
    MCTS(int simulations = 800, float c_puct = 1.0f,
         float dirichlet_alpha = 0.3f, float dirichlet_weight = 0.25f)
...
private:
    ...
    std::array<float, 256> prior_buffer_;
    std::array<uint32_t, 512> path_buffer_;
    ...
    static float get_policy_value(const chess::Move &move,
                                  const std::vector<float> &policy) {
        const int index = move.from() * 64 + move.to();
        return (index < static_cast<int>(policy.size())) ? policy[index]
                                                         : POLICY_EPSILON;
    }
};
