...
class TestChessLogicScientific:
    ...
    def test_incremental_hash_matches_recomputed_hash_after_castling_rights_change(self):
        pos = ccore.Position()
        pos.from_fen("r3k2r/8/8/8/8/8/8/R3K2R w KQkq - 0 1")

        for move in pos.legal_moves():
            if ccore.uci_of_move(move) == "e1f1":
                pos.make_move(move)
                break
        else:
            raise AssertionError("Expected e1f1 to be legal")

        recomputed = ccore.Position()
        recomputed.from_fen(pos.to_fen())
        assert pos.castling == recomputed.castling
        assert pos.hash == recomputed.hash
    ...
    def test_mate_in_one_mechanics(self):
        ...
        assert pos.result() == ccore.WHITE_WIN, f"After a1a8 the result should be WHITE_WIN, " f"but got {pos.result()}"
